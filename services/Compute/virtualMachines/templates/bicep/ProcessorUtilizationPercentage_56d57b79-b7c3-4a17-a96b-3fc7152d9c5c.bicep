...
param alertDescription string = 'Log Alert for Virtual Machine Processor Utilization Percentage'
...
@minLength(1)
param query string = 'InsightsMetrics | where Origin == "vm.azm.ms" | where Namespace == "Processor" and Name == "UtilizationPercentage" | summarize AggregatedValue = avg(Val) by bin(TimeGenerated, 15m), Computer, _ResourceId '
