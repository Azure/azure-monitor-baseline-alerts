<#
    .SYNOPSIS
    This script generates subscription-based policy templates based on YAML files containing alert configurations.
    ...
    .EXAMPLE
    .\generate-policies-full-arm-subscription.ps1
...
process {
    ...
    $alertsYaml | ForEach-Object -Parallel{
        ...
        foreach ($alert in $alertJsonObject) {
            ...
            $policyDirectory = "$($policyPathName)templates\policy-arm-subscription"
            ...
            If ($alert.type -eq "Metric") {
                if ($alert.type -eq "Metric" -and $alert.properties.criterionType -eq "StaticThresholdCriterion") {
                    $alertTemplate = Get-Content ".\policy\metric-static-full-arm-Subs.json"
                }
                if ($alert.type -eq "Metric" -and $alert.properties.criterionType -eq "DynamicThresholdCriterion") {
                    $alertTemplate = Get-Content ".\policy\metric-dynamic-full-arm-Subs.json"
                }
                ...
                Out-File -FilePath "$policyPathName\templates\policy-arm-subscription\$($policyFileName)_$($alert.guid).json" -InputObject $alertTemplate
            }

            # Generate policy templates for log alerts
            elseif ($alert.type -eq "Log") {
              $alertTemplate = Get-Content ".\policy\log-full-arm-Subs.json"
              $alertTemplate = $alertTemplate -replace "##TELEMETRY_PID##", 'pid-8bb7cf8a-bcf7-4264-abcb-703ace2fc84d'
              $alertTemplate = $alertTemplate -replace "##POLICY_NAME##", $alert.guid
              if ($alert.deployments.name -ne $null) {
                $alertTemplate = $alertTemplate -replace "##POLICY_DISPLAY_NAME##", $alert.deployments.name
            ...
            elseif ($alert.type -eq "ActivityLog") {
              if ($alert.properties.category -eq "Administrative") {
                $alertTemplate = Get-Content ".\policy\activity-administrative-full-arm-Subs.json"
              } elseif ($alert.properties.category -eq "ResourceHealth") {
                $alertTemplate = Get-Content ".\policy\activity-resourcehealth-full-arm-Subs.json"
              } elseif ($alert.properties.category -eq "ServiceHealth") {
                $alertTemplate = Get-Content ".\policy\activity-servicehealth-full-arm-Subs.json"
              }
    ...
    Write-Output "Subscription-based policy templates generated successfully."
}
